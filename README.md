...
- The default app page now loads the latest saved view-only management dashboard when saved reports are available.
- If no saved report exists yet, the default page shows a management-safe landing page without upload controls.
- Team upload and report generation are available only from `?view=team-upload`.
- Optional upload restriction is supported with Streamlit secret `UPLOAD_PASSCODE`.
